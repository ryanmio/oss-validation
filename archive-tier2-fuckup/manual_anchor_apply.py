"""Apply manual anchor overrides and recompute Tier-2 positional-accuracy stats.

Usage (defaults are usually fine):
    python -m oss_validation.manual_anchor_apply \
        --base data/processed/tier2_positional_accuracy.csv \
        --worksheets "data/processed/100 anchor worksheet - Sheet*.csv" \
        --out-final data/processed/tier2_final_with_manual.csv \
        --out-worst data/processed/tier2_anchor_audit_good_worst10.csv

The script is functionally identical to the ad-hoc inline snippets we were
running during the chat – but now lives as a proper reproducible module in the
repo.
"""
